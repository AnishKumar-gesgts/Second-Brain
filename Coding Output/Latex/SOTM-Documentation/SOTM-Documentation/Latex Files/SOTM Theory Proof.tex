...
For a robot to be able to shoot while moving, it must be able to correct for its current velocity to account for the transfering of its momentum to a ball that exits the robot/turret. To do this, I examined the idea of adding a target vector with the magnitude and direction of a stationary shoot to the robot's velocity vector, calculating a final magnitude and direction vector that represents the corrected vector of the trajectory of a ball. Using additional methods to account for elements that impact flight such as drag, it is possible to simulate, although it may not be 100\% accurate. By using efficiency constants for hardware construction and spin transfer, it is possible to determine the RPM of the ball based on the target exit velocity vector that uses kinematics to determine this value. Information like position, velocity and gyro readings can be obtained through robot-relevant odometry calculations, inherently specific to robot construction and drivetrain tracking.
...
% \[\|\vec{l}(x,t)\|cos(\phi)\hat{\imath} + \|\vec{l}(y,t)\|sin(\phi)\hat{\jmath}\]
